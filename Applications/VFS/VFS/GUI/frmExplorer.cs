...
namespace VFS.GUI
{
    public partial class frmExplorer : Form
    {
        ...
        private void TbPage_OnSelectedChanged(Tab.Element currentElement)
        {
            ...
            {
                this.lblOrdnerName.Text = "Ordner: " + (string.IsNullOrEmpty(currentElement.CurrentFile.GetParentDirectory().GetName()) ? "root" : currentElement.CurrentFile.GetParentDirectory().GetName());
            ...
            else
            {
                ...
                this.lblOrdnerName.Text = "Ordner: " + (string.IsNullOrEmpty(currentElement.CurrentDir.GetName()) ? "root" : currentElement.CurrentDir.GetName());
                this.lblSelectedFile.Text = "Ausgewählte Datei: -";
                this.lblSizeOfFile.Text = "Größer der Datei: -";
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                selctedTabPage.CurrentFileSystem.WriteAllText(this.txtNotepad.Text, this.currentFile.GetName(), selctedTabPage.CurrentDirectory, true);
            ...
            if (this.tbcFiles.SelectedTab != null)
            {
                ...
                if (currentTab.Selected != null)
                {
                    if (!currentTab.Selected.IsFile)
                    {
                        ...
                        {
                            if (currentTab.Selected.CurrentDir.GetParent() == null)
                                currentTab.CurrentFileSystem.RootDirectory.GetParent().Remove(currentTab.Selected.CurrentDir);
                            else
                                currentTab.Selected.CurrentDir.GetParent().Remove(currentTab.Selected.CurrentDir);
        ...
        private void öffnenToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.tbcFiles.SelectedTab != null)
            {
                ...
                {
                    IFile currentFile = selctedTabPage.Selected.CurrentFile;
                    if (currentFile != null)
                    {
        ...
        private void openFile(IFile file)
        {
            ...
            foreach (string extension in lstExtensions)
                if (file.GetName().ToLower().EndsWith(extension))
                    isImage = true;
            ...
            if (isImage)
            {
                try
                {
                    Image x = (Bitmap)((new ImageConverter()).ConvertFrom(file.GetBytes().ToArray()));
            ...
            string content = System.Text.Encoding.Default.GetString(file.GetBytes().ToArray());
            if (content == null)
                content = string.Empty;

            this.txtNotepad.Text = content;
            this.lblTxt.Text = "Datei: " + file.GetPath();
                ...
                if (currentTab.Selected != null && currentTab.Selected.IsFile)
                {
                    using (FolderBrowserDialog fld = new FolderBrowserDialog())
                    {
                        if (fld.ShowDialog(this) == DialogResult.OK)
                        {
                            try
                            {
                                System.IO.File.WriteAllBytes(System.IO.Path.Combine(fld.SelectedPath, currentTab.Selected.CurrentFile.GetName()), currentTab.Selected.CurrentFile.GetBytes().ToArray());
                            }
                            catch { }
                        }
                    }
        ...
        private void löschenToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (this.tbcFiles.SelectedTab != null)
            {
                ...
                if (currentTab.Selected != null && currentTab.Selected.IsFile)
                {
                    currentTab.Selected.CurrentFile.GetParentDirectory().GetFiles().Remove(currentTab.Selected.CurrentFile);
                    currentTab.CurrentFileSystem.Save();
                }
                else
                {
        ...
        private void zurückToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                Tab.vTabPage selctedTabPage = this.tbcFiles.SelectedTab as Tab.vTabPage;
                if (string.IsNullOrEmpty(selctedTabPage.CurrentDirectory.GetName()) && string.IsNullOrEmpty(selctedTabPage.CurrentDirectory.ToFullPath()))
                ...
                else if (selctedTabPage.CurrentDirectory.GetParent() != null)
                    selctedTabPage.CurrentDirectory = selctedTabPage.CurrentDirectory.GetParent();
            }
            else
                MessageBox.Show(this, "Sie haben kein Archiv ausgewählt!", "Kein Archiv ausgewählt", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
                ...
                if (!currentElement.IsFile)
                {
                    using (InputDialog ipd = new InputDialog())
                    {
                        if (ipd.ShowDialog(this, "Bitte geben Sie den neuen Namen ein!", "Namen eingeben", currentElement.Name) == DialogResult.OK)
                        {
                            ...
                            currentElement.CurrentDir.SetName(ipd.Result);
            ...
            if (this.tbcFiles.SelectedTab != null)
            {
                ...
                if (currentElement.IsFile)
                {
                    using (InputDialog ipd = new InputDialog())
                    {
                        ...
                        {
                            currentElement.Name = ipd.Result;
                            currentElement.CurrentFile.SetName(ipd.Result);
                            selectedTabPage.CurrentFileSystem.Save();
                        }
                    }
                }
