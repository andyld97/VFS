...
namespace VFS.GUI.Tab
{
    public class vTabPage : TabPage
    {
        ...
        public vTabPage(string path) : base(System.IO.Path.GetFileNameWithoutExtension(path))
        {
            ...
            this.currentFileSystem = new VFS(System.IO.Path.Combine(Application.StartupPath, "Log.log"), path, sc.MainCounter, sc.PackByte);
            this.CurrentFileSystem.RootDirectory = new Directory("");
        ...
        new public void Refresh()
        {
            ...
            foreach (IDirectory currentDir in this.currentDirectory.GetSubDirectories())
                this.lstElements.Add(new Element(currentDir.GetName(), Element.Type_.Directory, currentDir));

            foreach (IFile currentFile in this.currentDirectory.GetFiles())
                this.lstElements.Add(new Element(currentFile.GetName(), Element.Type_.File, null, currentFile));
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            foreach (Element currentElement in this.GetList())
            {
                ...
                e.Graphics.DrawImage(currentElement.IsFile ? Properties.Resources.File : Properties.Resources.Directory, new Rectangle(0, index * distance, 32, 32));
